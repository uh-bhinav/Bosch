...
>
> **Pull-Direction Optimization (feat/pull-direction, 2026-08-13/14)**: Milestones
...
> stage uses confirmed_undercut only. 26 new tests passing.
> **R1–R5 semantic correction complete (2026-08-14)**: `undercut_face_ids`=confirmed-only;
> `suspected_undercut_face_ids`=FAILED/SKIPPED; `boolean_no_interference_face_ids` tracked;
> `boolean_validation_complete` suitability gate; `validation_fallback` flag;
> 16 new semantic contract tests all passing.
> **Fix D/E (Boolean pool reduction + false-positive rendering, 2026-08-15)**: excluded
> perpendicular-dot faces (`|n·d| ≤ 0.01`) from Boolean candidate pool (all proxy undercuts
> are in this set by construction); removed expanded final pass (`boolean_check_all_core_side`,
> `final_direction_max_boolean_faces=150`); final pass now guaranteed cache hit; `no_interference`
> faces render as `accessible` (neutral gray) not pale green. +8 tests, 0 new failures.
> Milestone 5 (Docker regression matrix on Part1/Part3) remains open.
...
- [ ] **Milestone 5 (pull-direction M5) — Docker regression matrix on Part1/Part3 with OCC (2026-08-13; scope expanded 2026-08-15)**: Run `optimize_mold_direction()` on both real STEP files. **Assert after Fix D/E**: Part1+Z Boolean candidate pool ≤ 20 non-perp-dot faces (was ~230); `boolean_validation_complete=True` on the winner; direction result = +Z; wall-clock time < 30s. Also verify R1-R5 fields (`suspected_undercut_face_ids`, `boolean_no_interference_face_ids`, `validation_fallback`) in live API responses. Requires Docker + OCC.
