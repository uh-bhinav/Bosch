...
> expanded final pass `boolean_check_all_core_side=True` budget=150;
> 16 new semantic contract tests all passing. Milestone 5
...
- [ ] **Milestone 5 (pull-direction M5) — Docker regression matrix on Part1/Part3 with OCC (2026-08-13; scope expanded 2026-08-14)**: Run `optimize_mold_direction()` on both real STEP files with `hierarchical_search_enabled=True` and compare best direction, final score, Boolean op count, and wall-clock time against the pre-M1–M4 baseline. **Also verify R1–R5 fields** (`suspected_undercut_face_ids`, `boolean_no_interference_face_ids`, `boolean_validation_complete`, `validation_fallback`) in live API responses. API compatibility check (`/direction`, `/undercuts`, `/draft` return valid JSON with new fields). Performance comparison table in CHANGELOG.md. Requires Docker + OCC.
