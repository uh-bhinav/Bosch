...
| **Mock-test hygiene** | Mock-based tests (`occ_face=MagicMock()`) calling `detect_undercuts()`/`optimize_mold_direction()` without explicit `boolean_refine=False` stall for minutes against real pythonocc-core. | Partially mitigated by Bug G's `isinstance` guard. Still needs an audit pass across `test_undercut_detector.py` and `test_direction_optimizer.py`. |
