...
| Pull-direction search timing | `optimize_mold_direction(Part1.stp)` measured at 201s (parallelism=6) / 164.5s (parallelism=8, current default) in live, instrumented runs (2026-08-19) — O24's real subprocess-per-candidate parallelism is confirmed active and delivering a genuine 4.4-6.15x speedup, not bypassed. The remaining floor is each batch's single slowest candidate (~61-65s observed worst case) plus ~2.3-3.2s fresh-subprocess overhead per candidate; WHY specific directions cost that much is still not root-caused. One outlier run has been measured as high as ~29.6 minutes — **still the single largest live-demo risk**; budget for this or pre-compute results before presenting. |
