...
| ID | Issue | Impact |
|---|---|---|
| **F4** | `networkx==3.3` pinned in `requirements.txt`, imported nowhere. | Parting line uses a hand-rolled bounded DFS instead of graph algorithms. Resolved by Roadmap Phase 1.6. |
| — | `test_api_error_handling.py::test_parting_line_paths_payload_is_json_safe` fails for real (not the F5 import bug — surfaced once F5 was fixed). | Needs investigation. Found 2026-07-27 during Phase 0.4's Docker pytest run. |
